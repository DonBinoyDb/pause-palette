...
import { ArrowLeft, Upload, X, Trash2, Check, Eye, EyeOff, Info, Plus, Save } from "lucide-react";
...
export default function EditProductForm({ 
  ...
  const [fitGuideDescription, setFitGuideDescription] = useState(product.fitGuideDescription || "");
  const [fitGuideImage, setFitGuideImage] = useState(product.fitGuideImage || "");
  ...
  // Colors
  const savedColors = Array.isArray(product.colors) ? product.colors : [];
  const mainColorItem = savedColors.find((c: any) => c.isMain);
  const additionalColors = savedColors.filter((c: any) => !c.isMain);

  const [mainColorName, setMainColorName] = useState(mainColorItem ? mainColorItem.name : "Original");
  const [colors, setColors] = useState<{name: string, hex: string, images: string[]}[]>(additionalColors);
