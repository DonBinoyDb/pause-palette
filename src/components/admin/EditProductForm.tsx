...
export default function EditProductForm({ 
  ...
  // Form Data initialized with server-fetched props
  const [name, setName] = useState(product.name || "");
  const [slug, setSlug] = useState(product.slug || "");
  const [description, setDescription] = useState(product.description || "");
  const [price, setPrice] = useState(product.price?.toString() || "");
  const [collectionId, setCollectionId] = useState(product.collectionId || "");
  const [silhouetteOption, setSilhouetteOption] = useState(product.silhouetteOption || "Regular");
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-4 items-center">
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold text-red-500 hover:bg-red-50 rounded-full transition-colors"
          >
